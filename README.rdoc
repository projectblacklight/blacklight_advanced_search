...
[config[:solr_type]]
  The internal strategy for mapping from user-entered query to Solr. Defaults to "parsing_nesting". Other options (not recommended) are "dismax" (original dismax strategy with known problems, may go away soon) or "edismax" (may work with the edismax query parser, this function may be a work in progress?)
[config[:qt]] 
