...
module SolrQuerySpecHelper
  ...
  def local_param_match(query)
    ...
    query =~ /^ *_query_:\"\{([^}]+)\}(.*)" *$/
    expect(param_str = Regexp.last_match(1)).not_to be_nil
    expect(query = Regexp.last_match(2)).not_to be_nil
...
describe 'NestingParser' do
  describe 'Translating to Solr' do
    ...
    describe 'with custom qf' do
      it 'insists on dismax for nested query' do
      ...
      it 'insists on edismax for nested query' do
    ...
    describe 'with embeddable AND query' do
      ...
      it 'flattens to one dismax query' do
        local_param_match(@query) do |_params, query|
          expect(query).to eq('one +two +three')
        end
      end
    ...
    describe 'for pure negative' do
      it 'converts simple pure negative' do
      ...
      it 'converts pure negative AND' do
      ...
      it 'converts pure negative OR' do
      ...
      it 'converts crazy pure negative combo' do
    ...
    describe 'Single Query' do
      ...
      describe 'simple search' do
        ...
        it 'works without local params' do
